...
class FairBaseReport(object):
    ...
    def __init__(self, currency_prefix="$"):
        ...
        self._param_cols = ["low", "most_likely", "high", "constant", "mean", "stdev"]
    ...
    def _get_data_table(self, model):
        ...
        table = data.to_html(border=0, justify="left", classes="fair_metadata_table")
        return table
            ...
            pd.Series(
                {
                    ...
                    "PyFair Version": __version__,
                    "Type": type(self).__name__,
                }
            )
            .to_frame()
            .to_html(
                border=0, header=False, justify="left", classes="fair_metadata_table"
            )
        )
        return metadata
    ...
    def _get_overview_table(self, model_or_models):
        ...
        overview_df = risk_results.map(lambda x: self._format_strings["Risk"].format(x))
        overview_df.loc["Simulations"] = [
            "{0:,.0f}".format(len(model.export_results()))
            for model in model_or_models.values()
        ]
        # Add data
        overview_df.loc["Identifier"] = [
            model.get_uuid() for model in model_or_models.values()
        ]
        overview_df.loc["Model Type"] = [
            model.__class__.__name__ for model in model_or_models.values()
        ]
        # Export df to HTML and return
        overview_html = overview_df.to_html(
            border=0, header=True, justify="left", classes="fair_table"
        )
    ...
    def _get_model_parameter_table(self, model):
        ...
        param_df["distribution"] = [
            ...
            for target in param_df.index.values
        ]
        # Export table to html
        detail_table = param_df.to_html(
            border=0, header=True, justify="left", classes="fair_table", escape=False
        )
        return detail_table
        ...
        risk_df = pd.DataFrame(
            {
                ...
                "geo_mean": risk_df.apply(
                    lambda x: np.exp(np.mean(np.log(x[x > 0]))), axis=1
                ),
